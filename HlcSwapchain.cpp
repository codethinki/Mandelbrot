...
namespace hlc {
	...
	HlcSwapchain::HlcSwapchain(HlcDevice& device_ref, VkExtent2D extent)
		: device{ device_ref }, windowExtent{ extent } {
		init();
	}
	HlcSwapchain::HlcSwapchain(HlcDevice& device_ref, VkExtent2D window_extent, std::shared_ptr<HlcSwapchain> previous)
		: device{ device_ref }, windowExtent{ window_extent }, oldSwapchain{ previous } {
		init();
	...
	VkResult HlcSwapchain::acquireNextImage(uint32_t* image_index) {
		vkWaitForFences(
			device.device(),
			1,
	...
	VkResult HlcSwapchain::submitCommandBuffers(
	...
	VkExtent2D HlcSwapchain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
	...
	VkFormat HlcSwapchain::findDepthFormat() {
