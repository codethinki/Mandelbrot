...
void main(){
	float realZ = 0, imgZ = 0, realC = map(gl_FragCoord.x, 0, push.general.x, push.edges[0], push.edges[1]), imgC = map(gl_FragCoord.y, 0, push.general.y, push.edges[2], push.edges[3]);
	int i = -1;
	...
	float quotient = i / push.general[2];
