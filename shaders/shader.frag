...
void main(){		
	double realZ = 0, imgZ = 0, realC = map(gl_FragCoord.x, 0, push.general.x, push.edges.x, push.edges.y), imgC = map(gl_FragCoord.y, 0, push.general.y, push.edges.z, push.edges.w);
	
	int i = 0;
	while(i < push.general.z && realZ * realZ + imgZ * imgZ <= 4){
		double img = realZ * imgZ * 2;
